...
async function getTotalCommits(requests, contributor, cutoffDate) {
  ...
  repos.forEach((repo) => {
    // Log the structure of repo.data
    console.log(repo.data);

    // Check if repo.data is an array
    if (Array.isArray(repo.data)) {
      ...
      if (indexOfContributor !== -1) {
        const commits = repo.data[indexOfContributor].total;
        totalCommits += commits;
      }
    } else {
      console.error("repo.data is not an array:", repo.data);
    }
  });
